 ...
 */
- (void)downloadFile: (NSDictionary *)params  success: (WXModuleCallback)success failure: (WXModuleCallback)failure progress: (WXModuleCallback)progress{
    ...
    [BsFileManager downloadFile: params downloadSuccess:^(NSURLResponse *response, NSURL *filePath) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            // 设置进度条的百分比
            progress(@{@"progress": [NSNumber numberWithFloat:1.0 * downloadProgress.completedUnitCount / downloadProgress.totalUnitCount]});
        });
    }];
